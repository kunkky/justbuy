      ...
      try {
        ...
        if (!priceRange) {
          ...
          const filteredOrders = items.filter(
            (order) => parseInt(order.price) <= priceRange
          );
          return res.status(200).send({
            responseCode: "00",
            responseMessage: "Order Retrieved successfully",
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newProduct = new Products({
        ...
        discountRate,
        productRating,
        productCategories: productCategoriesArr,
        numberSold,
        productImages: productImagesArr,
        productDetails,
        owner_Id,
...
router.get("/getFeaturedProducts", bodyParse.json(), async(req, res) => {
    try {
...
router.post("/createBillingInfo", requireAuth, bodyParse.json(), async(req, res) => {
        const Schema = Joi.object({
           fullName : Joi.string().min(3).required(),
           email: Joi.string().email().required(),
           companyName: Joi.string().min(3),
           streetAddress: Joi.string().min(3).required(),
           apartment: Joi.string().min(3),
           townCity: Joi.string().min(3).required(),
    ...
    try {
        ...
        let billingInfo = await Billing.findOne({email})
        if (billingInfo) return res.status(400).send({
            responseCode: "96",
            responseMessage: "Billing information already exists",
            data: null
        })
        ...
        res.status(200).send({
            responseCode: "00",
            responseMessage: "Billing Info created successfully",
            data: billingInfo,
          });
        } catch (error) {
          res.status(500).send({
...
router.get("/getBillingInfo",   bodyParse.json(), async(req,res) => {
        try {
            let billingInfo = await Billing.find();
            if (!billingInfo) {
                return res.status(400).send({
                    responseCode: "96",
                    responseMessage: "No billing info could be fetched",
                    data: null
                })
            }
           return  res.status(200).send({
                responseCode: "00",
                responseMessage: "Billing Info fetched successfully.",
                data: billingInfo
            })
        } catch (error) {
            res.status(500).send({
